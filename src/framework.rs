use crate::{commands, config::get_config, database::User, types::{Context, Data, InvocationData, UnitResult}, utils::discord::reply::Reply};
use anyhow::Error;
use log::{error, info, warn};
use poise::{
    samples::register_in_guild, serenity_prelude::{self, ActivityData, FullEvent, Interaction}, Context as PoiseContext, EditTracker, Framework, FrameworkError, FrameworkOptions, PrefixFrameworkOptions
};
...
pub fn create_framework(data: Data) -> Framework<Data, Error> {
    ...
    Framework::builder()
        ...
        .setup(|ctx, ready, framework| {
            Box::pin(async move {
                for guild_id in &get_config().guild_ids {
...
async fn handle_with_invocation_data<'a>(
    ctx: &Context<'_>,
    data: &'a mut InvocationData
) -> bool {
